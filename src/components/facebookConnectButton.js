...
const FacebookConnectButton = ({ className, children, showData, onConnect, onDisconnect }) => {
	...
	useEffect(() => {
		!fieldValue && apiFetch({ url: constants.wordpress.access }).then((res) => {
	...
	return (
		<div>{
			facebookAccess
				...
				: <Button type='submit' disabled={loader} className={classNames('nfd-facebook-button--connect', `${className}--connect`)} onClick={() => connectFacebook()}>
					{loader && <Spinner />}
